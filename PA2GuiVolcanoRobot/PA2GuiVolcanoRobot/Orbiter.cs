...
namespace STLViewer
{
    class Orbiter
    {
        ...
        {
            return (float)(Math.Acos(val) * (180 / Math.PI));
        }
        float cos(float val)
        {
            return (float)Math.Cos(val * (Math.PI / 180));
        }
        float sin(float val)
        {
            return (float)Math.Sin(val * (Math.PI / 180));
        }

        public Quaternion eularToQuartenion(float roll, float pitch,float yaw)
        {
        ...
        public Orbit quartenionToOrbit(Quaternion q)
        {
            return new Orbit(acos(q.qw),q.qx,q.qy,q.qz);
        }
